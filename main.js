...
//     let downpayment = amount * (downpaymentPercent / 100);
//     let principal = amount - downpayment;
//     let totalPrincipal = principal;
//     principal -= onceCommissionAmount;
//     principal -= (monthlyCommissionPercent / 100 * principal);
...
//     let annuityEffectiveRate = Math.pow(1 + monthlyRate, 12) - 1;
...
//         let classicOverPayment = classicInterestExpense + (onceCommissionPercent / 100 * totalPrincipal);
...
//         let totalInsuranceCommission = monthlyInsuranceAmount * calculatePayments;
//         monthlyCommissionAmount += totalInsuranceCommission;
//         let totalCommissionPayments = monthlyCommissionAmount;

//         result.innerHTML = `Ежемесячный платёж (классическая схема): ${classicMonthlyPayment.toFixed(2)} грн. <br> Общие процентные расходы по кредиту (классическая схема): ${classicOverPayment.toFixed(2)} грн. <br> Переплата по кредиту (классическая схема): ${classicOverPayment.toFixed(2)} грн.<br> Эффективная процентная ставка (классическая схема): ${(classicEffectiveRate * 100).toFixed(6)}% <br> <br> Ежемесячный платёж (аннуитетная схема): ${annuityMonthlyPayment.toFixed(2)} грн. <br> Общие процентные расходы по кредиту (аннуитетная схема): ${annuityInterestExpense.toFixed(2)} грн. <br> Переплата по кредиту (аннуитетная схема): ${annuityOverPayment.toFixed(2)} грн.<br> Эффективная процентная ставка (аннуитетная схема): ${(annuityEffectiveRate * 100).toFixed(6)}%`;
...
//             let principalPayment = principal / calculatePayments;
//             let commissionPayment = totalCommissionPayments / calculatePayments;
//             remainingBalanceClassic -= principalPayment;
//             let totalPayment = principalPayment + interestPayment + annualInsuranceAmount + commissionPayment;
//             if (i === 1) firstPayment = totalPayment;
//             if (i === calculatePayments) lastPayment = totalPayment;

//             let row = `
//     <tr class="loan-table__name-list">
//       <td class="loan-table__item">${i} (${monthName})</td>
//       <td class="loan-table__item">${totalPayment.toFixed(2)}</td>
...
//             let commissionPayment = totalCommissionPayments / calculatePayments;
//             let totalPayment = annuityMonthlyPayment + commissionPayment;
//             annuityRemainingBalance -= principalPayment;
//             annuityRemainingBalance += interestPayment;
//             let row = `
//       <tr class="loan-table__name-list">
//         <td class="loan-table__item">${i} (${monthName})</td>
//         <td class="loan-table__item">${totalPayment.toFixed(2)}</td>
...
//         classicInterest.innerHTML = classicOverPayment.toFixed(2);
